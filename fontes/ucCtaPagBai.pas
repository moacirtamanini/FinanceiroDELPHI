...
uses
  ...
  Dialogs, ucPadrao, StdCtrls, Buttons, ExtCtrls, rtCtaPag, Mask,
  uFrame_Historico, rtTypes, rxCurrEdit, rxToolEdit;

type
  TfcCtaPagBai = class(TfcPadrao)
    Label1: TLabel;
...
constructor TfcCtaPagBai.Create(Aowner: TComponent; prID, prParcela, prMoedaTIP, prHistoCOD: Integer; prHistoDES: String);
begin
...
procedure TfcCtaPagBai.FormDestroy(Sender: TObject);
begin
  wCtaPag.Free;
  inherited;
end;

procedure TfcCtaPagBai.btnSairClick(Sender: TObject);
begin
  ModalResult := mrCancel;
  inherited;
end;

procedure TfcCtaPagBai.btnOKClick(Sender: TObject);
var
  wVlrSaldo: Currency;
begin
  inherited;
  ...
  begin
    ...
    MessageDlg('Favor informar todos os complementos de histórico!', mtWarning, [mbOk], 0);
    Exit;
  end;

  wVlrSaldo  := wCtaPag.fGetSaldoParcela(fParcela);
  if (wVlrSaldo <= 0) then
  begin
  ...
  begin
    if wCtaPag.fBaixarParcela(fParcela, fMoedaTIP, Frame_Histo.Histo, edDataBaixa.Date, edVlrPri.Value, edVlrJur.Value, edVlrMul.Value, edVlrDes.Value, edHistoDES.Text) then
    begin
      ModalResult := mrOk;
      Self.Close;
    end;
...
procedure TfcCtaPagBai.FormShow(Sender: TObject);
begin
  inherited;
  wCtaPag.ppID      := fID;
  edDataBaixa.Date  := fDataM.fDataHoraServidor;
  edVlrPri.Value    := wCtaPag.fGetSaldoParcela(fParcela);
  Frame_Histo.Histo := fHistoCOD;
  edHistoDES.Text   := fHistoDES;
end;

procedure TfcCtaPagBai.edDataBaixaKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  if (Key = #13) then
  begin
...
procedure TfcCtaPagBai.pCarregaHisto(Sender: TObject; Encontrou: Boolean);
begin
...
procedure TfcCtaPagBai.edHistoDESEnter(Sender: TObject);
begin
  inherited;
  if Pos('*', edHistoDES.Text) > 0 then
  begin
...
procedure TfcCtaPagBai.edHistoDESKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  if (Key = #13) then
  begin
...
procedure TfcCtaPagBai.edVlrPriChange(Sender: TObject);
begin
  inherited;
