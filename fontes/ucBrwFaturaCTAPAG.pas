...
type
  TfcBrwFaturaCTAPAG = class(TfcPadrao)
...
var
  fcBrwFaturaCTAPAG: TfcBrwFaturaCTAPAG;
...
procedure TfcBrwFaturaCTAPAG.btnSairClick(Sender: TObject);
begin
  ModalResult := mrCancel;
  inherited;
end;

procedure TfcBrwFaturaCTAPAG.pAtualizaGrade;
var
  wFiltros: String;
begin
  ...
  FIBQueryAtribuirSQL(qrGrade, 'select D.id, D.Empre, D.Codigo, D.EmitenteTIP, D.EmitenteCOD, D.Status, '+
                               ...
                               'D.HistoDES, D.MoedaTIP, D.CartaoTIP, D.CartaoCOD, D.CategoriaSAI, '+
                               ...
                               'C.descricao as categoriaSaiDES, '+
                               ...
                               'FROM CtaPagDocto D '+
                               'Left outer join categoria C on (D.empre = C.empre and D.categoriasai = C.codigo) '+
...
procedure TfcBrwFaturaCTAPAG.btnCarregarClick(Sender: TObject);
begin
  inherited;
...
procedure TfcBrwFaturaCTAPAG.FormShow(Sender: TObject);
begin
  inherited;
  pAtualizaGrade;
end;

procedure TfcBrwFaturaCTAPAG.DBGridGradeDblClick(Sender: TObject);
begin
  inherited;
  btnCarregarClick(Sender);
end;

procedure TfcBrwFaturaCTAPAG.FormKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  if (Key = #27) then Self.Close;
end;

procedure TfcBrwFaturaCTAPAG.ckQuitadasClick(Sender: TObject);
begin
  inherited;
  pAtualizaGrade;
end;

procedure TfcBrwFaturaCTAPAG.DBGridGradeDrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  inherited;
